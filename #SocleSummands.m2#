...
--	"lin1",
	...
	"test1",
	"golodTests",
	"Nonminimal", -- option for hasSocleSummand1 and kSi
	"TorMapk",
	"TorMapConjecture",
	"lin7",
	"dist",
...
hasSocleSummand1 = method(Options => {Nonminimal => false})
hasSocleSummand1(Module) := o-> M -> (
	...
	R := ring M;
	I := ideal vars R;
	--elapsedTime N := (0_R*M) : (I); -- this doesn't work if M is a cokernel module.
	if o.Nonminimal == false then (
	socR := ideal((0_R*R^1):I); 
	((gens (socR*ambient M)) % (I*M)) != 0)
       else (
   	N := (0_R*M) : (I); -- Bug: this doesn't work if M is a cokernel module, but does if M is a submodule !?
	((gens N) % (I*M)) != 0
    	)
)
...
kSi(R,7)
kSS(R,7)
--time F = res(k, LengthLimit => 12, FastNonminimal => true)
...
--toDo: kSi (Ideal, ZZ)
--kkSI--do both the res and the Koszul
kSi = method(Options=>{Nonminimal => false})
kSi ChainComplex := o -> F -> (
    ell := length F;
    apply(1 + ell, i-> (
	    if i<=1 then hasSocleSummand coker F.dd_(i+1) else
	    if i>1 and i<ell then hasSocleSummand(image F.dd_i) else
	    hasSocleSummand(image syz F.dd_ell)
	       ))
       )
...
kSi2 = method()
kSi2(Ring,ZZ) := List => (R,maxres) -> (
    --Let F be the resolution of the residue field of R.
    --returns list of Boolean, true for those image F.dd_i = i-th syzygy of k having a socle summand
    --the socle of F_i, which is (soc R)*F_i, is always in the image of F_(i+1) (the i-th syzygy). 
    --We have a socle summand of the i-th syzygy
    --if soc F_i is NOT contained in the image of (ideal vars R)*F_(i+1).
    --Output is a list of Boolean; i-th element is true if there is a socle summand in the i+1-st syzygy.
    v := ideal vars R;
    k := coker vars R;
    F := res(k, LengthLimit => maxres + 1);
    s := (ideal 0_R):v;--socle R, computed just once.
    -- socle of F_i = socle image F.dd_(i+1) 
    b := null;
    apply(toList(1..maxres), i-> (
	    b = gens(s*F_(i-1)) % (v*image F.dd_i) != 0;
	-- if true, 
	--then contains a minimal generator of im F.dd_(i+1)
    	<<b<<flush<<endl)
    )
)
*-	
